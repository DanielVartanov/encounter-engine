...
#  match('/stats/:game_id', :method => :get).to(:controller => :game_passings, :action => :index).name(:game_stats)
  ...
  match('/game_entries/accept/:id').to(:controller => :game_entries, :action => :accept).name(:accept) # принятие заявки
  match('/game_entries/reject/:id').to(:controller => :game_entries, :action => :reject).name(:reject) # отказ на регистрацию
