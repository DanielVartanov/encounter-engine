...
describe Invitation do
  describe "regular case, captain attempts to invite a new user" do
    before :each do
      ...
      @invitation = Invitation.new(:from_team => team, :recepient_email => user.email)
    end
  ...
  describe "captain attempts to invite himself :-)" do
    before :each do
      ...
      @invitation = Invitation.new(:from_team => team, :recepient_email => captain.email)
    end
  ...
  describe "captain attempts to invite someone twice" do
    before :each do
      ...
      user = create_user
      team = create_team :captain => captain
      Invitation.create!(:from_team => team, :recepient_email => user.email)
  ...
  describe "two captains invite same user" do
    before :each do
      ...
      user = create_user
      Invitation.create!(:from_team => team1, :recepient_email => user.email)

      @invitation = Invitation.new(:from_team => team2, :recepient_email => user.email)
    end
  ...
  describe "captain attempts to create invitation without providing recipient email" do
    before :each do
      ...
      @invitation = Invitation.new(:from_team => team, :recepient_email => nil)
    end
  ...
  describe "captain attempts providing an unexistant recipient email" do
    before :each do
      ...
      @invitation = Invitation.new(:from_team => team, :recepient_email => "unexistent@email.com")
    end
