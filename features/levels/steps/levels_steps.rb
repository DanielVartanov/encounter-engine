...
Given /добавляю задание "(.*)" в игру "(.*)"/ do |level_name, game_name|
  Given %{добавляю задание "#{level_name}"|enКраблеБумц в игру "#{game_name}"}
end
...
Given /добавляю задание "(.*)"\|(\S*) в игру "(.*)"/ do |level_name, correct_answer, game_name|
  ...
  When %{ввожу "#{correct_answer}" в поле "Правильные ответы"}
...
Given %r{^(.*) добавляет задание "(.*)"\|(\S*) в игру "(.*)"$}i do |user_name, level_name, correct_answer, game_name|
  Given %{я логинюсь как #{user_name}}
  Given %{добавляю задание "#{level_name}"|#{correct_answer} в игру "#{game_name}"}
end
