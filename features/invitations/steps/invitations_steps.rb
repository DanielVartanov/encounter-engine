When %r{высылаю пользователю (.*) приглашение вступить в команду}i do |recepient_name|
  ...
  Если %{иду по ссылке "Пригласить участников"}
  Если %{ввожу "#{recepient_name}" в поле "Пригласить нового участника"}
  Если %{нажимаю "Пригласить"}
...
Then %r{пользователь (.*) должен получить приглашение от команды (.*)}i do |nickname, team_name|
  ...
  То %{одно письмо с текстом "Вас пригласили вступить в команду #{team_name}" должно быть выслано на #{user_email}}
end
...
Then %r{пользователь (.*) не должен получить приглашение}i do |nickname|
  ...
  То %{никакие письма не должны быть высланы на #{user_email} }
end
