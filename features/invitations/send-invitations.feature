...
Сценарий: Рядовой член команды не видит ссылку "Пригласить участников"
    Допустим зарегистрирована команда "Mushrooms" под руководством noel@grib.kg
    И пользователь alisa@diesel.kg состоит в команде Mushrooms
...
Сценарий: Капитан пытается пригласить члена другой команды
    Допустим зарегистрирована команда "Плакучие Ивы" под руководством iv@diesel.kg
    И пользователь av@diesel.kg состоит в команде Плакучие Ивы
    И зарегистрирована команда "Mushrooms" под руководством noel@grib.kg
    Если я логинюсь как noel@grib.kg
    И захожу в комнату команды
    И иду по ссылке "Пригласить участников"
    И ввожу "av@diesel.kg" в поле "Пригласить нового участника"
    И нажимаю "Пригласить"
    То должен увидеть "Пользователь уже является членом одной из команд"
    И пользователь av@diesel.kg не должен получить приглашение
...
Сценарий: Капитан пытается пригласить члена своей же команды
    ...
    И нажимаю "Пригласить"
    То должен увидеть "Пользователь уже является членом одной из команд"
    И пользователь alisa@diesel.kg не должен получить приглашение
